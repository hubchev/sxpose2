...
{pstd}
{cmd:sxpose2} transposes the data, changing variables into observations and observations into variables. In contrast to the implemented command xpose, it also allows string variables to be transposed. It is build upon the well-received  user-written command sxpose and adds the possibility to keep variable names and/or variable labels. 

{title:Remarks}

{phang}
{cmd:sxpose2} has the same functionality as {cmd:xpose} and {cmd:sxpose} and goes beyond. It can to keep variable names like {cmd:xpose}, can transpose both string variables and numerical variables like {cmd:sxpose}, and does not drop integer values like {cmd:sxpose} when the first variable of the existing dataset is to be treated the new variable names of the transposed dataset, i.e, when both the {opt first:names} and the {opt force} options are specified. 

{marker options}{...}
{title:Options}

{phang}
{opt clear} is required and is supposed to remind you that the untransposed data will be lost (unless you have saved the data previously).

{phang}
{opt de:string}  specifies that {cmd:destring, replace} will be run on the new dataset in an attempt to convert variables that are unambiguously numeric in content to numeric. No force will be applied. See help on {help destring}. 

{phang}
{opt first:names} specifies that the first variable of the existing dataset is to be treated the new variable names of the transposed dataset. If the values are integer, a leading underscore will be added to the variable name. 
Other non conform variable names will be replaced by a numbered placeholder.

{phang}
{opt force} is required when the dataset contains numeric variables. Numeric values will be saved as strings using the format %12.0g by default. Other formats can be specified using the format() option. Please be aware that you may lose precision using this option. The {cmd: xpose} command allows to transpose a numeric dataset with higher precision.
...
{col 10}{inp:{stata list}}
		     ...
		     |-------------------------------------------|
		  1. |      v_b   Label-of-2     4       5     6 |
		  2. |      v_c   Label-of-3   c_1     c_2     9 |
		     +-------------------------------------------+
